...
const SelectModal = ({ handleSubmit }) => {
  ...
  const handleGuChange = async (event) => {
    ...
    try {
      const response = await axios.get(`http://localhost:8181/map/${encodeURIComponent(selectedValue)}`);
    ...
    <div id='selectModal' role='selectModal'>
        <div className="address-select-modal">
            ...
            <div className="dong-select">
                ...
                >
                <option value="" hidden>동</option>
                {dongOptions.map((option, index) => (
                    <option key={index} value={option.dong}>{option.dong}</option>
                ))}
                </select>
            </div>
        </div>
